...
(function () {
  ...
  var SHOP = {
    name: BRAND_CONF.name || "Ramos by Julia",
    whatsapp: BRAND_CONF.whatsapp || "",
    instagram: BRAND_CONF.instagram || "ramos_by_juliaaa",
    site: BRAND_CONF.domain || "ramosbyjulia.com",
    tagline: (BRAND_CONF.tagline && BRAND_CONF.tagline.en) || "Luxury Ramos Buchones & Floral Artistry"
  };
  ...
  function buPrice() {
    ...
    var b = window.state.bu;
    var extras = (b.greens ? 5 : 0) + (b.jewel ? 10 : 0) + (b.choc ? 5 : 0) + (b.sash ? 5 : 0);
  ...
  function renderSummary() {
    ...
    if (window.state.template !== 'classic') {
      ...
      if (window.state.bu.greens) rows.push(["🌿 Baby's Breath rim", '+$5']);
      rows.push(['🧻 ' + curWrap().label + ' wrap', 'Included']);
      rows.push(['🎀 ' + (rib.c ? rib.label + ' ribbon' : 'No ribbon'), rib.c ? 'Included' : '—']);
      if (window.state.bu.jewel) rows.push(['👑 Luxury Tiara / Crown', '+$10']);
      if (window.state.bu.choc) rows.push(['🦋 Gold Butterflies & Pearls', '+$5']);
      if (window.state.bu.sash) rows.push(['🎀 Personalized Sash · “' + SASH_TXT[window.state.bu.sash] + '”', '+$5']);
      ...
      var mck = '<div class="minickt">' +
        ...
        (window.state.bu.jewel ? '<div class="mtile"><span class="mshape">👑</span><span>Crown</span></div>' : '') +
        (window.state.bu.choc ? '<div class="mtile"><span class="mshape">🦋</span><span>Butterflies</span></div>' : '') +
      ...
      var msg = 'Hi Julia! My Ramo Buchón: ' + shapeName + ' shape, ' + pts.length + ' blooms (' + PATN[window.state.bu.pattern] + (window.state.bu.wall2 && window.state.bu.pattern === 'zones' ? ', double wall' : '') + ') — ' + Object.keys(tally).map(function (id) { return tally[id] + '× ' + (BYID[id] ? BYID[id].label : id); }).join(', ') + ', center: ' + (BYID[window.state.bu.center] ? BYID[window.state.bu.center].label : 'Center') + '. Wrap: ' + curWrap().label + '.';
      if (window.state.bu.greens) msg += " With Baby's Breath rim (+$5).";
      if (rib.c) msg += ' Ribbon: ' + rib.label + '.';
      if (window.state.bu.jewel) msg += ' Luxury Tiara Crown (+$10).';
      if (window.state.bu.choc) msg += ' Gold Butterflies & Pearls (+$5).';
      if (window.state.bu.sash) msg += ' Printed message sash: "' + SASH_TXT[window.state.bu.sash] + '" (+$5).';
      if (window.state.noteOn) msg += ' Printed gift card (+$' + NOTE_PRICE + ')' + (window.state.note.trim() ? ': "' + window.state.note.trim() + '"' : '') + '.';
      msg += ' Total ~$' + total() + '. For pickup in Gresham/Rockwood!';
      if (ordDM) {
        ordDM.href = 'https://ig.me/m/' + (SHOP.instagram || 'ramos_by_juliaaa');
        ordDM.onclick = function () {
    ...
    if (window.state.stems.length) {
      ord && ord.removeAttribute('disabled');
      var msg = "Hi Julia! My custom " + window.state.size + " bouquet: " + lines.join(', ') + '. Wrap: ' + curWrap().label + '.';
      if (rib.c) msg += ' Ribbon: ' + rib.label + '.';
      if (window.state.noteOn) msg += ' Printed gift card (+$' + NOTE_PRICE + ')' + (window.state.note.trim() ? ': "' + window.state.note.trim() + '"' : '') + '.';
      msg += ' Total ~$' + total() + '. For pickup in Gresham/Rockwood!';
      if (ordDM) {
  ...
  function init() {
    ...
    if (buX) {
      buX.innerHTML = [
        ...
        ['greens', "🌿 Baby's Breath", '+$5'],
        ['jewel', '👑 Tiara Crown', '+$10'],
        ['choc', '🦋 Butterflies', '+$5']
      ].map(function (a) {
  ...
  window.submitRequest = function () {
    ...
    if (box) {
      ...
      box.innerHTML = '<b>✅ Request ' + order.id + ' received!</b><br>Julia will confirm your bouquet and arrange your pickup slot in Gresham / Rockwood. Payment via Zelle or Cash App.';
    }
