/* ===== Ashley's Floral Boutique pixel engine + live effects ===== */
(function(){
'use strict';
var PAL={r:'#c9556f',R:'#e2919f',p:'#eeb7c5',P:'#f6d5dd',g:'#3c5934',l:'#7ea065',y:'#E0A21A',Y:'#f6d878',o:'#c96f4a',b:'#8a6440',B:'#8a6440',w:'#f7f3e8',W:'#ffffff',v:'#9b7fb8',c:'#7fb3c9',k:'#33312a',s:'#a9bb97'};
var SPR={
...
/* ---- order channels: Instagram DM + SMS ----
   This shop takes orders by DM or text only — there is no WhatsApp line, so
   there is deliberately no wa.me here. Instagram cannot carry a pre-filled
   message, so [data-msg] (the text button) is the one that gets the bouquet
   details baked in; [data-dm] just opens her inbox.
   No target=_blank on sms: — on desktop that leaves a dead blank tab behind. */
var PHONE='14058626632';            /* Ashley's Floral Boutique — call/text */
var IG='ashleys.floral.boutique';
window.SHOP_PHONE=PHONE; window.SHOP_IG=IG;
function smsHref(msg){
  /* "?&body=" is the form iOS honours; Android accepts it too. */
  return 'sms:+'+PHONE+'?&body='+encodeURIComponent(msg);
}
function dmHref(){return 'https://ig.me/m/'+IG;}
window.smsHref=smsHref; window.dmHref=dmHref;
document.querySelectorAll('[data-msg]').forEach(function(a){a.href=smsHref(a.dataset.msg);});
document.querySelectorAll('[data-dm]').forEach(function(a){a.href=dmHref();a.target='_blank';a.rel='noopener';});
...
(function(){
  ...
  var COLORS=['#FBE3EC','#FFF7FA','#E85C8C','#C22B5C','#FBE3EC'];
  var W,H,ps=[];
...
(function(){
  ...
  var last=0, COLORS=['#E85C8C','#c9a24b','#a9bb97'];
...
(function(){
  ...
  var BGS=['linear-gradient(160deg,#FBE3EC,#FFF7FA)','linear-gradient(160deg,#FFF7FA,#FFFFFF)','linear-gradient(160deg,#FBE3EC,#FFFFFF)','linear-gradient(160deg,#FFF7FA,#FBE3EC)'];
...
(function(){
  ...
  var ITEMS=phone?[
    ...
    {src:'assets/bouquets/b2.jpg',t:'card',x:.14,h:.32,r:-6,c:'75 ROSES · $270'},
    {src:'assets/bouquets/b1.jpg',t:'card',x:.86,h:.32,r:6,c:'SUNFLOWER RAMO · $155'}
  ]:[
    ...
    {src:'assets/bouquets/b2.jpg',t:'card',x:.095,h:.56,r:-6,c:'75 ROSES · $270'},
    {src:'assets/bouquets/b5.jpg',t:'card',x:.195,h:.40,r:4,c:'PINK CHARM · $65'},
    {src:'assets/bouquets/b3.jpg',t:'card',x:.805,h:.40,r:-4,c:'SOL Y ROSAS · $125'},
    {src:'assets/bouquets/b1.jpg',t:'card',x:.905,h:.56,r:6,c:'SUNFLOWER RAMO · $155'}
  ];
  ...
  function paint(){
    ...
    ITEMS.forEach(function(s){
      ...
      bctx.fillStyle='#FFFFFF'; bctx.fillRect(-fw/2,-ih/2,fw,ih);
  ...
  function frame(){
    ...
    ctx.fillStyle='rgba(232,92,140,.45)';
