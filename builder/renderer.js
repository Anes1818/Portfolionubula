...
function load(path){if(!cache.has(path)){const im=new Image(),promise=new Promise((resolve,reject)=>{im.onload=()=>resolve(im);im.onerror=()=>reject(new Error('Artwork failed to load: '+path));});cache.set(path,{im,promise});im.src=source(path);}return cache.get(path).promise;}
function image(path){const it=cache.get(path);if(!it||!it.im.complete||!it.im.naturalWidth)throw new Error('Artwork is not loaded: '+path);return it.im;}
async function ready(){const urls=new Set([NEBULA_META.wrap.back,NEBULA_META.wrap.front]);for(const v of Object.values(NEBULA_META.flowers)){urls.add(v.head);if(v.classicAvailable){urls.add(v.classicBloom);if(v.classicStem)urls.add(v.classicStem);}}for(const v of Object.values(NEBULA_META.finishes))urls.add(v.url);for(const v of Object.values(NEBULA_META.collars))urls.add(v.url);await Promise.all([...urls].map(load));return true;}
function tint(path,paper,color){
 ...
 const im=image(path),cv=makeCanvas(im.width,im.height),ctx=cv.getContext('2d',{willReadFrequently:true});ctx.drawImage(im,0,0);const d=ctx.getImageData(0,0,cv.width,cv.height),rgb=[1,3,5].map(i=>parseInt(color.slice(i,i+2),16));
...
function nodeDraw(ctx,n,{shadow=true,alpha=false}={}){ctx.save();ctx.translate(n.x,n.y);ctx.rotate(n.rot);if(!alpha){ctx.filter=n.bright!==1?'brightness('+n.bright+')':'none';if(shadow){ctx.shadowColor='rgba(42,36,24,.20)';ctx.shadowBlur=2.2;ctx.shadowOffsetY=1.5;}}ctx.drawImage(image(n.url),-n.w/2,-n.h/2,n.w,n.h);ctx.restore();}
...
function finishNodes(s,f,nodes){
 ...
 if(a.butterfly)add('butterfly','extra_butterfly_gold',classic?468:360+f.radius*.68,classic?f.rimY-157:390-f.radius*.48,65,-.17);
 if(a.diamond)add('diamond','diamond_pin',classic?358:360-f.radius*.1,classic?f.waist.y-4:389,27,.06);
 return list;
}
...
function classicStemMask(f){
 const cv=makeCanvas(),c=cv.getContext('2d'),wm=NEBULA_META.wrap;c.drawImage(image(wm.back),f.origin.x,f.origin.y,wm.width*f.scale,wm.height*(f.yScale||f.scale));
...
function drawArt(ctx,s,opts={}){
 ...
 if(sc.collar&&!opts.alphaOnly){const n=sc.collar;ctx.save();ctx.shadowColor='rgba(27,26,22,.16)';ctx.shadowBlur=8;ctx.shadowOffsetY=3;ctx.drawImage(tint(n.url,s.finishes.paper,s.finishes.tint),n.x-n.w/2,n.y-n.h/2,n.w,n.h);ctx.restore();}
 if(s.mode==='classic'&&!opts.alphaOnly){
  ...
  ctx.save();ctx.shadowColor='rgba(85,70,48,.15)';ctx.shadowBlur=12;ctx.shadowOffsetY=7;ctx.drawImage(back,f.origin.x,f.origin.y,wm.width*f.scale,wm.height*(f.yScale||f.scale));ctx.restore();
  const st=stemCanvas.getContext('2d');st.clearRect(0,0,720,820);
  for(const n of sc.nodes){const m=n.meta;if(!m.classicStem)continue;const scale=n.w/m.bloomWidth,[cx,cy]=m.bloomCenter;
   // Source-aligned head and neck; no detached, independently sheared stem.
   st.save();st.beginPath();st.rect(0,0,720,f.waist.y+8);st.clip();st.translate(n.x,n.y);st.rotate(n.rot);st.scale(scale,scale);st.translate(-cx,-cy);st.drawImage(image(m.classicStem),0,0);st.restore();
  }
  st.globalCompositeOperation='destination-in';st.drawImage(classicStemMask(f),0,0);st.globalCompositeOperation='source-over';ctx.drawImage(stemCanvas,0,0);ctx.drawImage(front,f.origin.x,f.origin.y,wm.width*f.scale,wm.height*(f.yScale||f.scale));
 }
...
function hit(sc,p){if(sc.frame.mode!=='classic'){let best=null,dist=Infinity;for(const n of sc.nodes){const d=Math.hypot(p.x-n.x,p.y-n.y),slot=sc.slots?.find(a=>a.slot===n.slot);if(d<dist&&d<(slot?.near||n.d)*.70){dist=d;best=n;}}return best;}for(const n of sc.nodes.slice().reverse()){const dx=p.x-n.x,dy=p.y-n.y,x=dx*Math.cos(n.rot)+dy*Math.sin(n.rot),y=-dx*Math.sin(n.rot)+dy*Math.cos(n.rot);if(Math.abs(x)>n.w/2||Math.abs(y)>n.h/2)continue;const a=alphaFor(n.url),ix=Math.floor((x/n.w+.5)*a.w),iy=Math.floor((y/n.h+.5)*a.h);if(ix>=0&&ix<a.w&&iy>=0&&iy<a.h&&a.data[(iy*a.w+ix)*4+3]>60)return n;}return null;}
...
async function blob(s,w,h,lang){const cv=exportCanvas(s,w,h,lang);return new Promise((resolve,reject)=>cv.toBlob(b=>b?resolve(b):reject(new Error('PNG export failed.')),'image/png'));}
g.NebulaRenderer={classicStemMask,source,load,ready,image,scene,drawArt,paint,view,worldPoint,hit,ghost,alphaCanvas,alphaReport,artBounds,exportCanvas,blob,wrapText,makeCanvas,collarNode};
})(window);
