/* ===== Ashley's Floral Boutique — Seasonal Events Engine v2 (ROBUSTE module) =====
...
var DEMO = false;         // live shop: themes activate automatically, no preview dropdown
...
var EVENTS=[
 {id:'valentine', icon:'💘', filter:'love', date:function(y){return new Date(y,1,14);},
  ...
  live:{en:"It's Valentine's Day! DM or text to see what's still available.",es:"¡Es San Valentín! Escríbenos por DM o mensaje para ver disponibilidad."},
 ...
 {id:'womensday', icon:'💜', filter:'arreglos', date:function(y){return new Date(y,2,8);},
  ...
  p:{en:"Bright, elegant arrangements for March 8 — ready for pickup or local delivery across Oklahoma City.",es:"Arreglos brillantes y elegantes para el 8 de marzo — listos para recoger o entrega local en Oklahoma City."},
  cta:{en:"💜 Shop the collection →",es:"💜 Ver la colección →"},
  live:{en:"Happy Women's Day! DM or text to reserve yours.",es:"¡Feliz Día de la Mujer! Escríbenos para reservar el tuyo."},
 ...
 {id:'mothersday', icon:'🌷', filter:'madre', date:function(y){return nth(y,4,0,2);},
  ...
  live:{en:"It's Mother's Day! DM or text to see what's still available.",es:"¡Es el Día de la Madre! Escríbenos para ver disponibilidad."},
 ...
 {id:'fathersday', icon:'🌻', filter:'buchon', date:function(y){return nth(y,5,0,3);},
  ...
  live:{en:"It's Father's Day! DM or text to reserve yours.",es:"¡Es el Día del Padre! Escríbenos para reservar el tuyo."},
 ...
 {id:'thanksgiving', icon:'🍂', filter:'arreglos', date:function(y){return nth(y,10,4,4);},
  ...
  live:{en:"Happy Thanksgiving! DM or text to reserve yours.",es:"¡Feliz Acción de Gracias! Escríbenos para reservar el tuyo."},
 ...
 {id:'christmas', icon:'🎄', filter:'eternas', date:function(y){return new Date(y,11,25);},
  ...
  live:{en:"Merry Christmas! DM or text to reserve yours.",es:"¡Feliz Navidad! Escríbenos para reservar el tuyo."},
...
function lang(){ try{ return (localStorage.getItem('ashleysLang')||'en')==='es'?'es':'en'; }catch(e){ return 'en'; } }
...
try{
  ...
  var ls=localStorage.getItem('ashleysEventDemo')||'';
...
/* ---- styles ---- */
var css=''+
'#evRibbon{display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:7px 14px;min-height:42px;background:#0D0C0F;color:#FFF7FA;padding:8px 18px;font-family:var(--px,monospace);font-size:10.5px;letter-spacing:.055em;line-height:1.4;position:relative;z-index:98;box-shadow:inset 0 -1px rgba(255,255,255,.09)}'+
'#evRibbon .ico{font-size:16px;line-height:1;animation:evPulse 3.2s ease-in-out infinite;display:inline-block}'+
...
'#evRibbon .cd{color:#E85C8C;white-space:nowrap}'+
'#evRibbon a.cta{color:#fff;text-decoration:underline;text-underline-offset:3px;white-space:nowrap}'+
'#evRibbon .pick{display:inline-flex;align-items:center;gap:7px;padding-left:12px;border-left:1px solid rgba(255,255,255,.24)}'+
...
'#evRibbon select option{background:#0D0C0F;color:#FFF7FA}'+
'.fbtn.ev-glow{outline:3px solid var(--rose);outline-offset:2px}'+
'#evTopStack{position:sticky;top:0;z-index:150;transition:box-shadow .28s ease}'+'#evTopStack.is-stuck{box-shadow:0 12px 30px -20px rgba(13,12,15,.6)}'+'#evTopStack #nav{position:static;top:auto;z-index:auto;background:#0D0C0F}'+
...
if(activeEv){
  ...
  bow.src='assets/bow-'+activeEv.id+'.png';
...
if(DEMO){
  ...
  sel.addEventListener('change',function(){
    try{ localStorage.setItem('ashleysEventDemo',sel.value); }catch(e){}
...
if(activeEv){ applyTheme(activeEv); addFalling(activeEv); }
