(function() {
    ...
    // ===== Firebase: single init lives in firebase-init.js (loaded first) =====
    ...
    // Legacy Algerian regions — replaced by Checkout.US_STATES (see lib/checkout.js)
    var wilayas = [];

    // ============== المتغيرات العامة ==============
    var orderModal = null;
    var cart = (window.Cart && window.Cart.items) || [];  // shared state lives in cart.js
    ...
    function renderProducts(products) {
        ...
        for (var i = 0; i < products.length; i++) {
            ...
            if (product.old_price && product.old_price > product.price) {
                ...
                oldPrice = '<small dir="ltr" class="old-price text-decoration-line-through text-muted">' + Money.format(product.old_price, 'USD') + '</small>';
            }
            ...
            var productCard = '<div class="col-6 col-md-4 col-lg-3 mb-4">' +
                ...
                oldPrice +
                '<p dir="ltr" class="current-price">' + Money.format(product.price, 'USD') + '</p>' +
                '</div>' +
    ...
    function renderSpecialOffers(offers) {
        ...
        for (var i = 0; i < offers.length; i++) {
            ...
            var offerHTML = '<div class="col-md-4">' +
                ...
                '<div class="offer-product-price">' + Money.format(product.price, 'USD') + '</div>' +
                '<div class="offer-product-old-price">' + Money.format(product.old_price, 'USD') + '</div>' +
        ...
        fetch('products.json')
            ...
            .then(function(products) {
                ...
                if (product) {
                    addToCart(
                        product.title,
                        Money.format(product.price, 'USD'),
                        product.price,
                        product.images,
                        product.id
                    );
    ...
    function loadCart() {
        ...
        Cart.load();
        cart = Cart.items;
        // Render whenever the shared cart changes (state -> view binding).
        Cart.onChange(function () { updateCartCount(); renderCart(); });
        updateCartCount();
        renderCart();
    }
    ...
    function renderCart() {
        ...
        if (cart.length === 0) {
            ...
            cartTotal.textContent = Money.format(0, 'USD');
            return;
        }
        ...
        for (var i = 0; i < cart.length; i++) {
            ...
            itemElement.innerHTML = '<div class="d-flex align-items-center">' +
                ...
                '<div class="cart-item-price mb-2">' + Money.format(item.price || 0, 'USD') + '</div>' +
        ...
        cartTotal.textContent = Money.format(total, 'USD');
    ...
    function updateQuantity(index, newQuantity) {
        Cart.setQty(index, newQuantity);
    }

    // إزالة منتج من السلة
    function removeFromCart(index) {
        var productName = (cart[index] && cart[index].name) || 'منتج';
        Cart.removeAt(index);
    ...
    function addToCart(productName, productPrice, priceValue, productImages, productId) {
        ...
        var image = Array.isArray(productImages) ? (productImages[0] || '')
                  : (typeof productImages === 'string' ? productImages : '');
        // Single source of truth (cart.js): it merges, persists, and notifies renderers.
        Cart.add({ id: id, name: name, price: price, image: image });
    ...
    function checkout() {
        ...
        // Fill the order form
        document.getElementById('productName').value = cart.length + ' item(s)';
        document.getElementById('productPriceValue').value = total;
        document.getElementById('productImageUrl').value = firstItem.image;

        document.getElementById('productNameDisplay').textContent = cart.length + ' item(s)';
        document.getElementById('productPrice').textContent = Money.format(total, 'USD');
        document.getElementById('productImage').src = firstItem.image;

        // Reset the form and refresh the live order summary
    ...
    // Recalculate and render the live order summary (subtotal, shipping, US sales tax, total)
    function updateOrderSummary() {
        var box = document.getElementById('orderSummaryBox');
        if (!box || typeof Checkout === 'undefined') return;
        ...
        box.innerHTML =
            '<div class="d-flex justify-content-between"><span>Subtotal</span><span>' + Money.format(summary.subtotal, 'USD') + '</span></div>' +
            '<div class="d-flex justify-content-between"><span>Shipping</span><span>' + (summary.freeShipping ? 'Free' : Money.format(summary.shipping, 'USD')) + '</span></div>' +
            '<div class="d-flex justify-content-between"><span>Tax</span><span>' + Money.format(summary.tax, 'USD') + '</span></div>' +
            '<hr class="my-1">' +
            '<div class="d-flex justify-content-between fw-bold"><span>Total</span><span>' + Money.format(summary.total, 'USD') + '</span></div>';
    }

    function submitOrder() {
        if (isSubmittingOrder) { return; }
        var billing = getOrderForm();
        var subtotal = cartSubtotal();

        // Validate the US billing/shipping details with the shared checkout engine
        var check = Checkout.validateCheckout({ billing: billing, method: 'standard', subtotal: subtotal });
        var errorsBox = document.getElementById('orderErrors');
        if (!check.valid) {
            var be = check.errors.billing || {};
            var msg = Object.keys(be).map(function(k) { return be[k]; }).join(' · ') || 'Please complete all required fields.';
            if (errorsBox) { errorsBox.textContent = msg; }
        ...
        var addr = Checkout.normalizeAddress(billing);

        // Build a US order object: USD money, US shipping address, neutral PENDING status
        var orderData = {
            id: 'ORD-' + Date.now(),
            products: cart,
            ...
            statusHistory: [{ status: Orders.STATUS.PENDING, at: new Date().toISOString(), by: 'storefront' }]
        };
        ...
        if (typeof db !== 'undefined') {
            db.collection('orders').add(orderData)
                .then(function(docRef) {
                    orderId = docRef.id;
                    // NEXT (payment): send orderId to the Stripe createPaymentIntent backend
                    // (lib/payments.js + functions.stripe.example.js), then confirm the card with
                    // Stripe.js before marking the order PAID.
                    return sendOrderEmail(orderId, addr, summary, 'Card (Stripe) — pending');
                })
                .then(function() {
                    showSuccessMessage(orderId, addr.fullName, addr.phone, summary.total);
            ...
            setTimeout(function() {
                try {
                    sendOrderEmail(orderId, addr, summary, 'Card (Stripe) — pending')
                        .then(function() {
                            showSuccessMessage(orderId, addr.fullName, addr.phone, summary.total);
                            clearCartAndResetForm();
                        })
    ...
    function sendOrderEmail(orderId, addr, summary, methodLabel) {
        // Build the product list for the notification email
        var productsList = '';
        for (var i = 0; i < cart.length; i++) {
            var item = cart[i];
            productsList += '<div style="margin-bottom: 10px; padding: 8px; border-bottom: 1px solid #eee;">' +
                ...
                '<strong>Price:</strong> ' + Money.format(item.price, 'USD') + ' <br>' +
                '<strong>Line total:</strong> ' + Money.format(item.price * item.quantity, 'USD') +
                '</div>';
        }

        // Send the notification email via EmailJS
        if (typeof emailjs !== 'undefined') {
            return emailjs.send("service_lc1q5k8", "template_a15g7yg", {
                ...
                wilaya: addr.stateName,
                address: addr.address1 + ', ' + addr.city + ', ' + addr.state + ' ' + addr.zip,
                total_price: Money.format(summary.total, 'USD'),
    ...
    function showSuccessMessage(orderId, fullName, phone, total) {
        var successMessage = '<div class="text-center" style="padding:6px 4px;">' +
            ...
            '<h3 style="margin-top:10px;font-weight:800;color:#2e7d32;font-size:clamp(1.4rem,5vw,2rem);">Your order is confirmed</h3>' +
            '<p style="font-size:clamp(1rem,3vw,1.2rem);font-weight:600;color:#2e7d32;margin:8px 0 2px;">We will email your receipt and shipping updates</p>' +
            '<p style="font-size:1.05rem;font-weight:700;color:#1b5e20;margin:0 0 4px;direction:ltr;">' + phone + '</p>' +
            '<p style="font-size:1.1rem;font-weight:700;color:#2e7d32;margin:4px 0 12px;">Thank you for your order</p>' +
            ...
            '<div><strong>Total:</strong> ' + Money.format(total, 'USD') + '</div>' +
            '</div>' +
            '<a href="https://wa.me/213656360457?text=' + encodeURIComponent(
                'Question about order ' + orderId + '\nName: ' + fullName + '\nItems: ' + cart.length + '\nTotal: ' + Money.format(total, 'USD') + '\nPhone: ' + phone
            ) + '" class="btn whatsapp-contact-btn mt-2 w-100" target="_blank">' +
            '<i class="bi bi-whatsapp"></i> Chat on WhatsApp (optional)' +
            '</a>' +
            '</div>';
    ...
    // Populate the US state dropdown and refresh totals when it changes
    function populateWilayas() {
        var stateSelect = document.getElementById('orderState');
        if (!stateSelect) return;

        stateSelect.innerHTML = '<option value="">Select state…</option>';
        if (typeof Checkout !== 'undefined' && Checkout.US_STATES) {
            Object.keys(Checkout.US_STATES).forEach(function(code) {
                var option = document.createElement('option');
                option.value = code;
                option.textContent = Checkout.US_STATES[code];
    ...
    function initializeCarousels() {
        setTimeout(function() {
            ...
            for (var i = 0; i < carousels.length; i++) {
                ...
                // العروض الخاصة تُدار بشكل منفصل في initOfferProducts (تمرير أبطأ + ��قط عند الظهور)
    ...
    function initializePage() {
        try {
            ...
            if (document.getElementById('orderState')) {
                populateWilayas();
            }
